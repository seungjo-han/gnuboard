...
if(!trim($_POST['token']) || !$comment_token || $comment_token != $_POST['token']) {
    die('올바른 방법으로 이용해 주십시오.');
}

// 090710
if (substr_count($wr_content, "&#") > 50) {
    die('내용에 올바르지 않은 코드가 다수 포함되어 있습니다.');
}
...
if($comment_min > 0 || $comment_max > 0) {
 
    $comment_cnt = mb_strlen(na_get_text($wr_content), 'UTF-8'); 
 
    if ($comment_min > 0 && $comment_min > $comment_cnt) {
        die('댓글은 이모티콘 등을 제외하고 '.$comment_min.'글자 이상 쓰셔야 합니다.');
    } else if ($comment_max > 0 && $comment_max < $comment_cnt) {
        die('댓글은 이모티콘 등을 제외하고 '.$comment_max.'글자 이하로 쓰셔야 합니다.');
    }
}
...
if ($is_guest) {
    if ($wr_name == '')
        die('이름은 필히 입력하셔야 합니다.');
    if(!chk_captcha())
        die('자동등록방지 숫자가 틀렸습니다.');
}
...
if (empty($wr['wr_id']))
    die("글이 존재하지 않습니다.".PHP_EOL."글이 삭제되었거나 이동하였을 수 있습니다.");
...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    if ($tmp_point + $board['bo_comment_point'] < 0 && !$is_admin)
        die('보유하신 포인트('.number_format($member['mb_point']).')가 없거나 모자라서 댓글쓰기('.number_format($board['bo_comment_point']).')가 불가합니다.'.PHP_EOL.PHP_EOL.'포인트를 적립하신 후 다시 댓글을 써 주십시오.');
    ...
    if ($comment_id) {
        ...
        if (!$reply_array['wr_id'])
            die('답변할 댓글이 없습니다.'.PHP_EOL.PHP_EOL.'답변하는 동안 댓글이 삭제되었을 수 있습니다.');
        ...
        if (strlen($reply_array['wr_comment_reply']) == 5)
            die('더 이상 답변하실 수 없습니다.'.PHP_EOL.PHP_EOL.'답변은 5단계 까지만 가능합니다.');
        ...
        if (!$row['reply'])
            ...
            die('더 이상 답변하실 수 없습니다.'.PHP_EOL.PHP_EOL.'답변은 26개 까지만 가능합니다.');
        else
...
else if ($w == 'cu') // 댓글 수정
{
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['mb_id'] === $group['gr_admin']) { // 자신이 관리하는 그룹인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                die('그룹관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        } else
            die('자신이 관리하는 그룹의 게시판이 아니므로 댓글을 수정할 수 없습니다.');
        ...
        if ($member['mb_id'] === $board['bo_admin']) { // 자신이 관리하는 게시판인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                die('게시판관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        } else
            die('자신이 관리하는 게시판이 아니므로 댓글을 수정할 수 없습니다.');
    } else if ($member['mb_id']) {
        if ($member['mb_id'] !== $comment['mb_id'])
            die('자신의 글이 아니므로 수정할 수 없습니다.');
    } else {
        if( !($comment['mb_id'] === '' && $comment['wr_password'] && check_password($post_wr_password, $comment['wr_password'])) )
            die('댓글을 수정할 권한이 없습니다.');
    }
    ...
    $row = sql_fetch($sql);
    if ($row['cnt'] && !$is_admin)
        die('이 댓글와 관련된 답변댓글이 존재하므로 수정 할 수 없습니다.');
