...
// 코멘트 출력
$sql_common = "from {$write_table} where wr_parent = '{$wr_id}' and wr_is_comment = '1' $na_cmt_where ";

//$sql = " select count(*) as cnt " . $sql_common;
//$row = sql_fetch($sql);
$total_count = (int)$write['wr_comment']; //$row['cnt'];

$crows = isset($boset['na_crows']) ? (int)$boset['na_crows'] : 0;
...
// 댓글 정렬
$cob = isset($cob) ? $cob : '';
$boset['na_cob'] = isset($boset['na_cob']) ? $boset['na_cob'] : '';
...
if($cob == 'new') {
	...
	$start_page = 1;
	$cmt_orderby = 'as_star_score desc, wr_comment, wr_comment_reply';
} else {
	$cob = 'old';
